...
def fit(root, report, uid, checkpoint, destination):
    ...
    for case in cases:
        ...
        rows.append({"p": [predictions[case["id"]]["probabilities"][k] for k in keys],
                     "label": keys.index(case["label"]), "task": case["family"], "keys": keys,
                     "source": case["family"], "inference_temperature": 1.0,
                     # Kev filters on this eligibility marker. Our declared fit population includes all variants.
                     "variant": "clean", "fez_variant": case["variant"]})
    temperature = fit_temperature(rows, aggregation="macro", points=81)
    if not math.isfinite(temperature) or not .25 <= temperature <= 4:
        raise ValueError("temperature fitter returned an invalid value")
    adjusted = [{**predictions[c["id"]], "probabilities": dict(zip(row["keys"],
                 probabilities_at_temperature(row, temperature).tolist()))} for c, row in zip(cases, rows)]
    result = {"raw_checkpoint_sha256": entry["sha256"], "dataset_sha256": report["dataset_sha256"],
              "benchmark_manifest_sha256": benchmark.file_hash(Path(root) / "manifest.json"),
              "temperature": temperature, "fit_cases": len(rows), "fit_split": "calibration",
              "method": "Kev macro-family NLL; 81 log-spaced temperatures in [0.25, 4]; all calibration variants",
              "probability_floor": 1e-9,
              "at_grid_boundary": temperature in (.25, 4.0),
              "before": fez.score(cases, miner["predictions"]), "after": fez.score(cases, adjusted)}
    ...
    if result["before"]["accuracy"] != result["after"]["accuracy"]:
        raise ValueError("temperature unexpectedly changed accuracy; inspect numerical ties before saving")
    fez.stage(entry, destination)
    meta.temperature = temperature
    meta.extra["fez_temperature_fit"] = {k: result[k] for k in (
        "raw_checkpoint_sha256", "dataset_sha256", "benchmark_manifest_sha256", "fit_cases", "fit_split", "method")}
    head = destination / "head.pt"
    head.chmod(0o600)
    write_meta(destination, meta)
    head.chmod(0o444)
    result["checkpoint_sha256"] = fez.checkpoint_hash(destination)
    benchmark.write_private(destination / "calibration.json", json.dumps(result, indent=2, allow_nan=False) + "\n")
    return result
...
def main():
    ...
    try:
        result = fit(args.benchmark, json.loads(Path(args.report).read_text()), args.uid, args.checkpoint, args.out)
        print(json.dumps({"checkpoint": str(Path(args.out).resolve()), "temperature": result["temperature"],
                          "fit_cases": result["fit_cases"], "sha256": result["checkpoint_sha256"]}))
