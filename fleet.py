"""Persistent Fez miners and validator for a trusted private LAN. No chain writes."""
import argparse
...
def initialize(out, benchmark_path, checkpoint, host, port, miner_ports):
    import benchmark
    ...
    validator_key = Keypair.create_from_seed(validator_seed)
    shared = {"validator": endpoint, "validator_hotkey": validator_key.ss58_address,
              "base_revision": wire.BASE_REVISION, "initial_sha256": entry["sha256"],
              "training_sha256": digest(data / "miner-training.jsonl")}
    members = {}
    for uid, miner_port in enumerate(miner_ports, 1):
        directory = out / f"miner-{uid}"; directory.mkdir(mode=0o700)
        seed = "0x" + secrets.token_hex(32)
        key = Keypair.create_from_seed(seed)
        members[uid] = {"hotkey": key.ss58_address, "port": miner_port}
        wire.write_json(directory / "config.json", {**shared, "uid": uid, "seed": seed, "port": miner_port})
        shutil.copyfile(data / "miner-training.jsonl", directory / "miner-training.jsonl")
        fez.stage(entry, directory / "reference")
        for name in ("fleet.py", "rehearsal.py", "fez.py", "kev_runner.py", "requirements-model.txt", "requirements-rehearsal.txt", "FLEET.md"):
        ...
        archive.chmod(0o600)
    wire.write_json(validator_dir / "config.json", {**shared, "seed": validator_seed, "members": members,
...
def train_candidate(config, directory, job, runtime, device):
    ...
    seed = int(hashlib.sha256((config["validator_hotkey"] + config["seed"] + job["round_id"]).encode()).hexdigest()[:8], 16) % 2**31
...
def main():
    ...
    try:
        if args.role == "init":
            out = initialize(args.out, args.benchmark, args.checkpoint, args.host, args.port, args.miner_ports)
