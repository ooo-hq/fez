...
class TestnetTest(unittest.TestCase):
    def test_registration_and_publication_fail_closed(self):
        ...
        state = {"NetworksAdded": True, "Uids": {"validator": 0, "miner-a": 1, "miner-b": 2},
                 "Keys": {0: "validator", 1: "miner-a", 2: "miner-b"},
                 "ValidatorPermit": [True, False, False], "SubnetOwnerHotkey": "validator",
                 "CommitRevealWeightsEnabled": True, "MinAllowedWeights": 1, "MaxWeightsLimit": 65535,
                 "WeightsSetRateLimit": 100, "WeightsVersionKey": 0, "LastUpdate": [100, 0, 0]}
        ...
        class Chain:
            ...
            def execute(self, intent, wallet, **kwargs):
                ...
                assert intent.uids == [1, 2] and intent.weights == [.75, .25]
                assert kwargs["wait_for_finalization"] and kwargs["retries"] == 0
                self.calls += 1
                return ExtrinsicResult(True, block_hash="0xabc", extrinsic_id="251-1", data={"reveal_round": 900})
            def plan(self, intent, wallet):
                from bittensor.intents.plan import Plan
                return Plan(op=intent.op, summary="fixture", signer="hotkey", signer_address="validator",
                            fee=None, effects=[], warnings=[])

        sub = Chain()
        self.assertEqual(t.preflight(config, sub)["validator_uid"], 0)
        self.assertEqual(sub.last_block, 250)
        for field, value in (("network", "finney"), ("netuid", 0), ("netuid", True), ("netuid", "553")):
            bad = copy.deepcopy(config); bad["chain"][field] = value
            with self.assertRaises(ValueError): t.preflight(bad, sub)
        fresh = copy.deepcopy(config); fresh["chain"]["netuid"] = 777
        sub.expected_netuid = 777
        self.assertEqual(t.preflight(fresh, sub)["netuid"], 777)
        sub.expected_netuid = 553
        state["Uids"]["miner-a"] = 4
        with self.assertRaisesRegex(ValueError, "registration"): t.preflight(config, sub)
        state["Uids"]["miner-a"] = 1
        state["Keys"][1] = "replacement"
        with self.assertRaisesRegex(ValueError, "registration"): t.preflight(config, sub)
        state["Keys"][1] = "miner-a"
        report = {"chain": config["chain"], "identities": {"1": "miner-a", "2": "miner-b"},
                  "miners": [{"uid": 1, "status": "evaluated", "skill": .75},
                             {"uid": 2, "status": "evaluated", "skill": .25}]}
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(t.publish_round(config, work, report, sub, wallet, publish=True), receipt)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(t.publish_round(config, work, report, sub, wallet, publish=True)["status"], "rate_limited")
            self.assertEqual(sub.calls, 1)
            state["LastUpdate"][0] = 100
            empty = {**report, "miners": [{"uid": 1, "status": "rejected", "skill": 1}]}
            self.assertEqual(t.publish_round(config, work, empty, sub, wallet, publish=True)["status"], "no_weights")
            self.assertEqual(sub.calls, 1, "never invent weights for failed miners")
            bad = {**report, "miners": [{"uid": 8, "status": "evaluated", "skill": .5}]}
            with self.assertRaises(ValueError): t.publish_round(config, work, bad, sub, wallet, publish=True)
            old_local_report = {"miners": report["miners"]}
            with self.assertRaises(ValueError): t.publish_round(config, work, old_local_report, sub, wallet, publish=True)
            state["Uids"]["miner-b"] = None
            with self.assertRaises(ValueError): t.publish_round(config, work, report, sub, wallet, publish=True)
            state["Uids"]["miner-b"] = 2
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(sub, "execute", return_value=ExtrinsicResult(False, message="rejected")):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(sub, "execute", side_effect=KeyboardInterrupt):
                with self.assertRaises(KeyboardInterrupt): t.publish_round(config, work, report, sub, wallet, publish=True)
            self.assertFalse((work / "chain-receipt.json").exists())
            self.assertEqual(t.publish_round(config, work, report, sub, wallet, publish=True)["status"], "unknown")
            self.assertEqual(sub.calls, 1)
        state["Weights"] = [(1, 49151), (2, 16384)]
        state["LastUpdate"][0] = 251
        sub.block = 252
        record = {**t.preflight(config, sub), "block": 250, "chain_write": True,
                  "requested_weights": {"1": .75, "2": .25}}
        self.assertTrue(t.verify_receipt(config, record, sub)["weights_verified"])
        state["LastUpdate"][0] = 100
        self.assertFalse(t.verify_receipt(config, record, sub)["weights_verified"], "old weights must not prove this commit revealed")
        sub.expected_netuid = 777
        with tempfile.TemporaryDirectory() as tmp:
            receipt = t.publish_round(fresh, Path(tmp), {**report, "chain": fresh["chain"]}, sub, wallet, publish=True)
            self.assertEqual(receipt["status"], "committed")
    ...
    def test_wallet_signing_and_bundles_keep_keys_out(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for index in range(3):
                ...
                key = Wallet(**wallet).regenerate_hotkey(seed=bytes([index + 1]) * 32, suppress=True).hotkey
                roster.append({"uid": index + 8, "wallet": wallet, "hotkey": key.ss58_address})
            data = root / "data"; benchmark.build(data, seed=42)
            checkpoint = root / "checkpoint"; checkpoint.mkdir()
            for name in fez.ARTIFACT_FILES: (checkpoint / name).write_bytes(b"fixture")
            out = fleet.initialize(root / "fleet", data, checkpoint, "127.0.0.1", 8900, [8901, 8902],
                                   {"chain": {"network": "test", "netuid": 777},
                                    "validator": roster[0], "miners": roster[1:]})
