...
def main():
    ...
    with redirect_stdout(sys.stderr):
        ...
        if (checkpoint.meta.lora, checkpoint.meta.head_dim, checkpoint.meta.weights_dtype) != (16, 256, "fp32"):
        ...
        tokenizer, model = checkpoint.load(args.device, LoadOptions(dtype=torch.float32, backend="torch"))
        sync(args.device)
        model_load_ms = (time.perf_counter() - loading_started) * 1000
        predictions = []
        for request in requests:
            payload = SystemOneRequest(state=request["state"], questions={"decision": request["question"]})
            ...
            sync(args.device)
            predictions.append({"id": request["id"], "probabilities": dict(zip(metadata[0]["keys"], probabilities)),
                                "elapsed_ms": (time.perf_counter() - started) * 1000})
        package = importlib.metadata.distribution("kev")
        origin = json.loads(package.read_text("direct_url.json") or "{}")
        runtime = {"torch": torch.__version__, "kev": package.version,
                   "kev_commit": origin.get("vcs_info", {}).get("commit_id"),
                   "python": sys.version.split()[0], "backend": "torch", "dtype": "fp32",
                   "temperature": checkpoint.meta.temperature, "model_load_ms": model_load_ms}
