...
class CalibrationTest(unittest.TestCase):
    def test_calibration_uses_only_matching_raw_development_predictions(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            data = root / "data"; benchmark.build(data, seed=553)
            ...
            write_meta(source, Meta(base=fez.BASE, head={"fixture": torch.zeros(1)}, temperature=1.0))
            original = fez.checkpoint_hash(source)
            predictions = []
            for case in cases:
                keys = fez.options(case["question"])
                wrong = next(k for k in keys if k != case["label"])
                predictions.append({"id": case["id"], "elapsed_ms": 1,
                                    "probabilities": {k: .97 if k == wrong else .03 / (len(keys) - 1) for k in keys}})
            report = {"dataset_sha256": hashlib.sha256(json.dumps(cases, sort_keys=True, allow_nan=False).encode()).hexdigest(),
                      "miners": [{"uid": 1, "sha256": original, "status": "evaluated",
                                  "runtime": {"temperature": 1.0}, "predictions": predictions}]}
            ...
            self.assertEqual((source / "adapter_model.safetensors").read_bytes(),
                             (root / "fitted/adapter_model.safetensors").read_bytes())
            with self.assertRaises(FileExistsError):
                calibrate.fit(data, report, 1, source, root / "fitted")
            for changed, pattern in (("dataset", "dataset"), ("temperature", "temperature"), ("hash", "checkpoint")):
